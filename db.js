...
async function create_product_table(client) {
    await client.query('CREATE TABLE IF NOT EXISTS public."Produto"'+
                        '('+
                        '    "cod" bigint NOT NULL,'+
...
async function create_sale_table(client) {
    await client.query('CREATE SEQUENCE IF NOT EXISTS public.Vendas_id_seq INCREMENT 1 START 1 MINVALUE 1 MAXVALUE 2147483647 CACHE 1;')
    .then(
        () => {
...
async function create_product_sale_table(client) {
    await client.query('CREATE TABLE IF NOT EXISTS public."ProdutoVendido"'+
                        '('+
